...
if [ "$UVICORN_RELOAD_ENABLED" = "true" ]; then
    ...
    UVICORN_RELOAD_ARGS="--reload --reload-dir /opt/dfc-tg/src --reload-dir /opt/dfc-tg/assets --reload-include *.ftl"
else
    echo "Uvicorn will run without reload"
fi

exec uvicorn src.__main__:application --host "${APP_HOST}" --port "${APP_PORT}" --factory --use-colors --timeout-graceful-shutdown 25 ${UVICORN_RELOAD_ARGS}