...
chmod +x /opt/dfc-tg/scripts/*.sh 2>/dev/null || true

# Создать необходимые папки если их нет
mkdir -p /opt/dfc-tg/logs
mkdir -p /opt/dfc-tg/backups
mkdir -p /opt/dfc-tg/assets

# Установить права на папки
chmod 755 /opt/dfc-tg/logs
chmod 755 /opt/dfc-tg/backups
chmod 755 /opt/dfc-tg/assets
...
echo -e "${YELLOW}  Удаляем старый образ dfc-tg:local...${NC}"
docker rmi dfc-tg:local -f 2>/dev/null || true
...
while true; do
    ...
    case $PROXY_CHOICE in
        1)
            ...
            if [ -f "$CADDY_FILE" ]; then
                ...
                if grep -q "https://${APP_DOMAIN}" "$CADDY_FILE"; then
                    ...
                    echo "        reverse_proxy * http://dfc-tg:5000" >> "$CADDY_FILE"
                    echo "}" >> "$CADDY_FILE"
                ...
                echo "        reverse_proxy * http://dfc-tg:5000"
                echo "}"
            fi
            break
            ;;
        2)
            ...
            echo ""
            echo "    location / {"
            echo "        proxy_pass http://dfc-tg:5000;"
...
cd /opt/dfc-tg && docker compose up -d --build
...
# Ждем БД
echo -ne "  База данных: "
if wait_for_healthy "dfc-tg-db" 30; then
    echo -e "${GREEN}✓${NC}"
else
...
# Ждем Redis
echo -ne "  Redis: "
if wait_for_healthy "dfc-tg-redis" 30; then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${YELLOW}запускается...${NC}"
fi
...
echo -ne "  Бот: "
sleep 10
if docker ps --format '{{.Names}}' | grep -q "^dfc-tg$"; then
    # Проверяем логи на успешный запуск
    if docker compose logs dfc-tg 2>&1 | grep -q "Uvicorn running"; then
        echo -e "${GREEN}✓${NC}"
    else
        echo -e "${YELLOW}запускается...${NC}"
        sleep 10
...
echo ""
if docker compose logs dfc-tg 2>&1 | grep -qi "error\|exception\|failed" | head -1; then
    echo -e "${YELLOW}⚠ Обнаружены предупреждения в логах. Проверьте: docker compose logs dfc-tg${NC}"
fi

# Проверяем создание DEV пользователя
if docker compose logs dfc-tg 2>&1 | grep -q "DEV user.*created automatically"; then
...
echo -e "  ${YELLOW}docker compose logs -f dfc-tg${NC} - логи бота (Ctrl+C для выхода)"
