...
if [ -z "$BACKUP_FILE" ]; then
  ...
  echo ""
  echo "Пример: $0 ./backups/dfc-tg_backup_2025-01-03.sql"
  exit 1
fi
...
echo "=== Восстановление базы данных dfc-tg ==="
echo "Файл: $BACKUP_FILE"
echo ""

# Проверяем доступность контейнера
if ! docker ps --format '{{.Names}}' | grep -q '^dfc-tg-db$'; then
  echo "ОШИБКА: Контейнер dfc-tg-db не запущен!"
  echo "Запустите: docker compose up -d dfc-tg-db"
  exit 1
fi
...
for i in {1..30}; do
  if docker exec dfc-tg-db pg_isready -U dfc-tg -d dfc-tg > /dev/null 2>&1; then
...
docker stop dfc-tg dfc-tg-taskiq-worker dfc-tg-taskiq-scheduler 2>/dev/null || true
...
cat "$BACKUP_FILE" | docker exec -i dfc-tg-db psql \
  -U "dfc-tg" \
  -d "dfc-tg" \
  --single-transaction \
  --set ON_ERROR_STOP=1 \
...
if [ $RESULT -eq 0 ]; then
  ...
  docker exec dfc-tg-db psql -U dfc-tg -d dfc-tg -c "
  ...
  echo ""
  echo "Запускаем бота..."
  docker start dfc-tg dfc-tg-taskiq-worker dfc-tg-taskiq-scheduler 2>/dev/null || true
