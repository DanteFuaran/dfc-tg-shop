...
async def on_toggle_language(
    ...
    settings = await settings_service.get()
    old_value = settings.features.language_enabled
    new_value = not old_value
    
    if new_value:
        # ВКЛЮЧАЕМ мультиязычность
        # Сохраняем текущий bot_locale для возможности восстановления
        settings.features.previous_locale = settings.bot_locale
        settings.features.language_enabled = True
        
        # Триггерим отслеживание изменений
        settings.features = settings.features
        
        await settings_service.update(settings)
        
        # Перезагружаем настройки и обновляем middleware_data
        updated_settings = await settings_service.get()
        dialog_manager.middleware_data[SETTINGS_KEY] = updated_settings
        
        # Очищаем временный translator_runner чтобы i18n провайдер создал новый
        # на основе user.language (т.к. теперь language_enabled=True)
        dialog_manager.middleware_data.pop("translator_runner", None)
        
        # Принудительно обновляем диалог - i18n провайдер применит язык пользователя
        dialog_manager.show_mode = ShowMode.EDIT
        await dialog_manager.bg().update({})
        
        logger.info(f"{log(user)} Enabled multilingual mode, switched to user language: {user.language}")
    else:
        # ОТКЛЮЧАЕМ мультиязычность
        # Восстанавливаем предыдущий bot_locale (тот что был до включения мультиязычности)
        previous_locale = settings.features.previous_locale
        if previous_locale:
            settings.bot_locale = previous_locale
            logger.info(f"{log(user)} Restored bot_locale to {previous_locale}")
        else:
            # Если previous_locale не был сохранен, используем текущий bot_locale
            logger.info(f"{log(user)} No previous_locale saved, keeping {settings.bot_locale}")
        
        settings.features.language_enabled = False
        ...
        # Очищаем временный translator_runner чтобы i18n провайдер создал новый
        # на основе bot_locale (т.к. теперь language_enabled=False)
        dialog_manager.middleware_data.pop("translator_runner", None)
        
        # Принудительно обновляем диалог - i18n провайдер применит bot_locale
        dialog_manager.show_mode = ShowMode.EDIT
        await dialog_manager.bg().update({})
        
        logger.info(f"{log(user)} Disabled multilingual mode, switched to admin language: {updated_settings.bot_locale}")
...
async def on_language_select(
    ...
    if locale_code in locale_map:
        ...
        if "original_locale" not in dialog_manager.dialog_data:
            ...
            dialog_manager.dialog_data["original_multilang"] = current_settings.features.language_enabled
        ...
        # АВТОМАТИЧЕСКИ ОТКЛЮЧАЕМ мультиязычность при выборе языка
        if settings_copy.features.language_enabled:
            settings_copy.features.language_enabled = False
            settings_copy.features = settings_copy.features  # Trigger change tracking
            logger.info(f"{log(user)} Auto-disabled multilingual mode - preview with {selected_locale}")
        ...
        # Принудительно обновляем диалог для применения нового языка и состояния мультиязычности
...
async def on_language_cancel(
    ...
    # Восстанавливаем исходные значения если были изменения
    original_locale = dialog_manager.dialog_data.get("original_locale")
    original_multilang = dialog_manager.dialog_data.get("original_multilang")
    
    if original_locale is not None or original_multilang is not None:
        settings = await settings_service.get()
        settings_copy = settings.model_copy()
        
        # Восстанавливаем исходный язык и мультиязычность
        if original_locale is not None:
            settings_copy.bot_locale = original_locale
        if original_multilang is not None:
            settings_copy.features.language_enabled = original_multilang
            settings_copy.features = settings_copy.features  # Trigger change tracking
        
        dialog_manager.middleware_data[SETTINGS_KEY] = settings_copy
        
        # Очищаем временный translator_runner - i18n провайдер создаст новый
        dialog_manager.middleware_data.pop("translator_runner", None)
        
        logger.info(f"{log(user)} Cancelled language selection, restored to {original_locale}, multilang={original_multilang}")
...
async def on_language_apply(
    ...
    if pending_locale:
        ...
        # Мультиязычность уже отключена в on_language_select, но проверяем на всякий случай
        if settings_copy.features.language_enabled:
            settings_copy.features.language_enabled = False
            settings_copy.features = settings_copy.features  # Trigger change tracking
            logger.info(f"{log(user)} Auto-disabled multilingual mode - all users will use {pending_locale}")
        
        await settings_service.update(settings_copy)
        
        # Обновляем текущего пользователя в middleware_data
        user_copy = user.model_copy()
        user_copy.language = pending_locale
        
        # Обновляем в middleware_data
        updated_settings = await settings_service.get()
        dialog_manager.middleware_data[SETTINGS_KEY] = updated_settings
        dialog_manager.middleware_data[USER_KEY] = user_copy
        ...
        # КРИТИЧНО: Пересоздаем TranslatorRunner с новым языком ПЕРЕД переходом на главную страницу
        # Это гарантирует что меню настроек сразу отобразится на выбранном языке
        container: AsyncContainer = dialog_manager.middleware_data[CONTAINER_KEY]
        hub: TranslatorHub = await container.get(TranslatorHub)
