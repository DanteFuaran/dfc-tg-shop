...
from src.bot.states import TelegramReferral, DashboardSettings
...
async def on_reward_preset_select(
    ...
    if widget.widget_id == "manual_input_dummy":
        ...
        await dialog_manager.switch_to(TelegramReferral.REWARD_MANUAL_INPUT)
        return
...
async def on_reward_input(
    ...
    if text.isdigit():
        ...
        await dialog_manager.switch_to(TelegramReferral.REWARD)
    else:
        await notification_service.notify_user(
            user=user,
            payload=MessagePayload(i18n_key="ntf-referral-invalid-reward"),
...
async def on_submenu_cancel(
    ...
    await dialog_manager.switch_to(TelegramReferral.MAIN)
...
async def on_invite_message_input(
    ...
    await dialog_manager.switch_to(TelegramReferral.INVITE_MESSAGE)
