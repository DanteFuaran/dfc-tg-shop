...
from src.bot.states import TelegramPlans
...
async def on_plan_create(
    ...
    await dialog_manager.switch_to(state=TelegramPlans.CONFIGURATOR)
...
async def on_plan_select(
    ...
    await sub_manager.switch_to(state=TelegramPlans.CONFIGURATOR)
...
async def on_plan_delete(
    ...
    if is_double_click(dialog_manager, key=f"delete_confirm_{plan.id}", cooldown=10):
        ...
        await dialog_manager.start(state=TelegramPlans.MAIN, mode=StartMode.RESET_STACK)
        return
...
async def on_description_delete(
    ...
    """Удалить описание - установить pending_plan_description в пустую строку, будет применено при Accept."""
    user: UserDto = dialog_manager.middleware_data[USER_KEY]
    ...
    await dialog_manager.switch_to(TelegramPlans.DESCRIPTION)
...
async def on_cancel_tag(
    ...
    await dialog_manager.switch_to(TelegramPlans.CONFIGURATOR)
...
async def on_tag_delete(
    ...
    await dialog_manager.switch_to(TelegramPlans.TAG)
...
async def on_duration_select(
    ...
    await sub_manager.switch_to(state=TelegramPlans.PRICES)
...
async def on_duration_input(
    ...
    await dialog_manager.switch_to(state=TelegramPlans.DURATIONS)
...
async def on_price_input(
    ...
    adapter.save(plan)
    await dialog_manager.switch_to(state=TelegramPlans.PRICES)
...
async def on_squads(
    ...
    await dialog_manager.switch_to(state=TelegramPlans.SQUADS)
...
async def on_internal_squads_click(
    ...
    await dialog_manager.switch_to(state=TelegramPlans.INTERNAL_SQUADS)
...
async def on_external_squads_click(
    ...
    await dialog_manager.switch_to(state=TelegramPlans.EXTERNAL_SQUADS)
...
async def on_external_squad_select(
    ...
    # Refresh dialog to show updated button states
    await dialog_manager.switch_to(TelegramPlans.EXTERNAL_SQUADS)
...
async def on_cancel_configurator(
    ...
    await dialog_manager.switch_to(TelegramPlans.MAIN)
...
async def on_confirm_plan(  # noqa: C901
    ...
    if "pending_plan_description" in dialog_manager.dialog_data:
        ...
        plan_dto.description = None if not pending_desc else pending_desc
