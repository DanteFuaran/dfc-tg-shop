...
class ErrorMiddleware(EventTypedMiddleware):
    ...
    async def middleware_logic(
        ...
        # Пропускаем dialog-исключения дальше к специализированному обработчику
        ...
        if isinstance(error, TelegramBadRequest):
            ...
            # message was deleted - сообщение было удалено пользователем между попытками редактирования
            if "chat not found" in error_str or "message was deleted" in error_str:
                return await handler(event, data)
        ...
        logger.exception(
            f"Error in bot: {error_type_name}: {error_message.as_html()}\n"
