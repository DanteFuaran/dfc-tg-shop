...
class DashboardDB(StatesGroup):
    ...
    CLEAR_USERS_CONFIRM = State()  # Окно подтверждения очистки пользователей
...
class DashboardTelegram(StatesGroup):
    MAIN = State()
    ADMINS = State()
    ADVERTISING = State()
    EXTRA_DEVICES = State()
...
class TelegramReferral(StatesGroup):
    MAIN = State()
    LEVEL = State()
    REWARD = State()
...
class TelegramGateways(StatesGroup):
    MAIN = State()
    SETTINGS = State()
    FIELD = State()
    CURRENCY = State()
...
class TelegramNotifications(StatesGroup):
    MAIN = State()
    USER = State()
    SYSTEM = State()


class TelegramPlans(StatesGroup):
    MAIN = State()
    CONFIGURATOR = State()
    NAME = State()
    DESCRIPTION = State()
