...
async def process_subscription_purchase(
    ...
    """
    Обрабатывает покупку подписки (основная логика).
    
    Эта функция может вызываться как из TaskIQ задачи (асинхронно),
    так и напрямую (синхронно) для оплаты с баланса.
    
    Args:
        skip_redirect: Если True, не выполняет редирект на экран успеха
                      (используется для синхронного вызова при оплате с баланса)
    """
    logger.info(f"[process_subscription_purchase] Started: transaction={transaction.payment_id}")
    ...
    logger.info(
        f"[process_subscription_purchase] user={user.telegram_id}, "
    ...
    try:
        ...
        # ========== RENEW: продлеваем подписку ==========
            ...
            # Продлеваем срок действия активных доп. устройств вместе с подпиской
            # Но не продлеваем устройства, помеченные на удаление
            if current_extra_devices > 0:
                active_purchases = await extra_device_service.get_active_by_subscription(subscription.id)
                renewed_count = 0
                for purchase in active_purchases:
                    # Пропускаем устройства на удалении
                    if not purchase.pending_deletion:
                        await extra_device_service.renew_purchase(purchase.id, duration_days=plan.duration)
                        renewed_count += 1
                
                if renewed_count > 0:
                    logger.info(
                        f"[RENEW] Extended {renewed_count} extra device purchases for '{user.telegram_id}' "
                        f"by {plan.duration} days (skipped {len(active_purchases) - renewed_count} pending deletion)"
                    )
        ...
        # Для оплаты балансом (skip_redirect=True) редирект делается в handlers.py
        # Для других методов оплаты (внешние шлюзы) делаем редирект здесь
        if not skip_redirect and transaction.gateway_type != PaymentGatewayType.BALANCE:
        ...
        logger.info(f"[process_subscription_purchase] Completed for '{user.telegram_id}'")
...
async def purchase_subscription_task(
    ...
    """
    Обрабатывает покупку подписки (TaskIQ задача).
