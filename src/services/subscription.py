...
class SubscriptionService(BaseService):
    ...
    async def has_used_trial(self, user_telegram_id: int) -> bool:
        ...
        Учитываются ВСЕ подписки (активные, истекшие, удаленные), чтобы
        пользователь не мог получить пробную подписку повторно.
        """
        conditions = and_(
    ...
    def apply_sync(target: T, source: Union[SubscriptionDto, RemnaSubscriptionDto]) -> T:
        from src.core.enums import SubscriptionStatus  # noqa: PLC0415
        
        # Не синхронизируем статус если подписка деактивирована или удалена
        # Эти статусы устанавливаются локально и не должны перезаписываться RemnaWave
        preserve_local_status = hasattr(target, 'status') and target.status in [
        ...
        # Исключаем поле status из синхронизации если нужно сохранить локальный статус
        if preserve_local_status:
            common_fields.discard('status')
            logger.debug(f"Preserving local status '{target.status}' - not syncing from RemnaWave")
