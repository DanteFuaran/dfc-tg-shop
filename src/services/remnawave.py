...
class RemnawaveService(BaseService):
    ...
    async def sync_user(self, remna_user: RemnaUserDto, creating: bool = True) -> None:
        ...
        if not subscription:
            ...
            if non_imported_subscriptions:
                ...
                logger.info(f"No subscription found for '{user.telegram_id}', creating IMPORT")

                # Определяем теги: в Remnawave — IMPORT_xxx, в боте — Импорт
                original_tag = remna_subscription.tag or ""
                if original_tag.startswith("IMPORT_"):
                    import_tag_remnawave = original_tag
                    original_tag_clean = original_tag[7:]
                    import_tag_display = f"IMPORT({original_tag_clean})"
                    should_update_remnawave_tag = False
                elif original_tag == IMPORTED_TAG:
                    import_tag_remnawave = original_tag
                    import_tag_display = "IMPORT"
                    should_update_remnawave_tag = False
                elif original_tag:
                    import_tag_remnawave = f"IMPORT_{original_tag}"
                ...
                # Получаем переведённое название "Импорт"
                i18n = self.translator_hub.get_translator_by_locale(locale=user.language)
                import_name = i18n.get("frg-import-name")

                # Параметры из Remnawave
                user_device_limit = remna_user.hwid_device_limit or remna_subscription.device_limit or 3
                traffic_limit_gb = format_bytes_to_gb(remna_user.traffic_limit_bytes) if remna_user.traffic_limit_bytes else remna_subscription.traffic_limit

                if remna_user.expire_at:
                    remaining = remna_user.expire_at - datetime_now()
                    duration_days = max(1, remaining.days)
                else:
                    duration_days = -1

                # Определяем тип плана
                tl = traffic_limit_gb if traffic_limit_gb and traffic_limit_gb > 0 else 0
                dl = user_device_limit if user_device_limit and user_device_limit > 0 else 0
                if tl and dl:
                ...
                temp_plan = PlanSnapshotDto(
                    ...
                    type=plan_type,
                    traffic_limit=traffic_limit_gb if traffic_limit_gb else -1,
                    device_limit=user_device_limit,
                    duration=duration_days,
                    traffic_limit_strategy=remna_user.traffic_limit_strategy or remna_subscription.traffic_limit_strategy or "NO_RESET",
                    internal_squads=remna_subscription.internal_squads if remna_subscription.internal_squads else [],
                ...
                # Меняем тег в Remnawave на IMPORT_xxx если нужно
                if should_update_remnawave_tag:
                    try:
                        ...
                        logger.info(
                            f"Changed Remnawave tag from '{original_tag}' to '{import_tag_remnawave}' "
                            f"for user '{user.telegram_id}'"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to update Remnawave tag for '{user.telegram_id}': {e}"
                        )

                logger.info(
                    f"Created IMPORT subscription '{import_tag_display}' ({import_name}) "
                    f"for user '{user.telegram_id}', device_limit={user_device_limit}, duration={duration_days} days"
                )

                # Перечитываем подписку с присвоенным ID из БД
                subscription = await self.subscription_service.get_current(telegram_id=user.telegram_id)
        ...
        if subscription:
            ...
            if old_tag != new_tag and new_tag:
                ...
                if matching_plan and matching_plan.is_active:
                    ...
                    if matching_plan:
                        logger.debug(
                            f"Found plan with tag '{new_tag}' but it's inactive, creating IMPORT subscription"
                        )
                    else:
                        logger.debug(
                            f"No plan found with tag '{new_tag}', creating IMPORT subscription"
                        )

                    # Определяем теги для Remnawave и для отображения в боте
                    if new_tag.startswith("IMPORT_"):
                        import_tag_remnawave = new_tag
                        original_tag = new_tag[7:]
                        import_tag_display = f"IMPORT({original_tag})"
                        should_update_remnawave_tag = False
                    else:
                        import_tag_remnawave = f"IMPORT_{new_tag}"
                        import_tag_display = f"IMPORT({new_tag})"
                        should_update_remnawave_tag = True
                    ...
                    # Параметры из Remnawave
                    user_device_limit = remna_user.hwid_device_limit or 3
                    traffic_limit_gb = format_bytes_to_gb(remna_user.traffic_limit_bytes) if remna_user.traffic_limit_bytes else -1
                    ...
                    plan_snapshot = PlanSnapshotDto(
                        ...
                        device_limit=user_device_limit,
                        duration=duration_days,
                        traffic_limit_strategy=remna_user.traffic_limit_strategy or "NO_RESET",
                        internal_squads=remna_subscription.internal_squads if remna_subscription.internal_squads else [],
                        external_squad=remna_subscription.external_squad,
                    )

                    subscription.plan = plan_snapshot
                    subscription.tag = import_tag_display
                    subscription.traffic_limit = traffic_limit_gb
                    subscription.device_limit = user_device_limit
                    subscription.traffic_limit_strategy = remna_user.traffic_limit_strategy or "NO_RESET"
                    ...
                    if should_update_remnawave_tag:
                        try:
                            ...
                            logger.info(
                                f"Changed Remnawave tag from '{new_tag}' to '{import_tag_remnawave}' "
                                f"for user '{user.telegram_id}'"
                            )
                        except Exception as e:
                            logger.warning(
                    ...
                    logger.info(
                        f"Created IMPORT subscription '{import_tag_display}' for user '{user.telegram_id}', "
                        f"device_limit={user_device_limit}, duration={duration_days} days"
                    )

                    # Сохраняем и выходим — не запускаем apply_sync который перезапишет тег
    ...
    async def handle_user_event(self, event: str, remna_user: RemnaUserDto) -> None:  # noqa: C901
        ...
        if event == RemnaUserEvent.CREATED:
            if remna_user.tag and (remna_user.tag == IMPORTED_TAG or remna_user.tag.startswith("IMPORT_")):
                await self.sync_user(remna_user)
            else:
                logger.debug(
                    f"Created RemnaUser '{remna_user.telegram_id}' "
                    f"is not tagged as '{IMPORTED_TAG}' or 'IMPORT_*', skipping sync"
                )
            return
