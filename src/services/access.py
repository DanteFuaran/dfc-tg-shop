...
class AccessService(BaseService):
    ...
    async def is_access_allowed(self, aiogram_user: AiogramUser, event: TelegramObject) -> bool:  # noqa: C901
        ...
        # DEV всегда имеет доступ, даже если пользователя нет в базе
        if aiogram_user.id == self.config.bot.dev_id:
            # Логируем только в DEBUG режиме, чтобы не засорять логи
            logger.debug(f"Access allowed for DEV user '{aiogram_user.id}'")
            return True
        ...
        if not user:
            if mode == AccessMode.INVITED and await self.referral_service.is_referral_event(
                ...
                logger.debug(f"Access allowed for referral event for user '{aiogram_user.id}'")
                return True
        ...
        if user.is_privileged:
            # Логируем только в DEBUG режиме для привилегированных пользователей
            logger.debug(f"Access allowed for user '{user.telegram_id}' (privileged)")
            return True
        ...
        if mode == AccessMode.PUBLIC:
            # Логируем только в DEBUG режиме для публичного доступа
            logger.debug(f"Access allowed for user '{user.telegram_id}' (mode: PUBLIC)")
            return True
        ...
        if mode == AccessMode.INVITED:
            # Логируем только в DEBUG режиме для приглашенных
            logger.debug(f"Access allowed for user '{user.telegram_id}' (mode: INVITED)")
            return True
