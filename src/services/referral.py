...
class ReferralService(BaseService):
    ...
    async def get_rewards_total_by_referral_id(
        self,
        referral_id: int,
        reward_type: ReferralRewardType,
    ) -> int:
        """Get cumulative rewards earned from a specific referral (never decreases)."""
        total = await self.uow.repository.referrals.sum_rewards_by_referral_id(
            referral_id,
            reward_type,
        )
        return total

    async def get_pending_rewards_amount(
        self,
        telegram_id: int,
        reward_type: ReferralRewardType,
    ) -> int:
        """Get sum of rewards that have not been issued yet."""
        total_amount = await self.uow.repository.referrals.sum_pending_rewards_by_user(
            telegram_id,
            reward_type,
        )
        logger.debug(
            f"Retrieved pending rewards amount as '{total_amount}' "
            f"for user '{telegram_id}' for type '{reward_type.name}'"
        )
        return total_amount

    async def withdraw_pending_rewards(
        self,
        telegram_id: int,
        reward_type: ReferralRewardType,
        ...
        """
        Withdraw pending rewards and return total amount.
        If amount is specified, withdraws only up to that amount.
        Otherwise, withdraws all pending rewards.
        """
        rewards = await self.uow.repository.referrals.get_pending_rewards_by_user(
            telegram_id,
            reward_type,
        )
        ...
        # If amount is specified, withdraw only up to that amount
        if amount is not None:
            ...
            for reward in rewards:
                ...
                # Calculate how much to withdraw from this reward
                to_withdraw = min(reward.amount, amount - withdrawn_amount)
                
                if to_withdraw == reward.amount:
                    # Mark entire reward as issued
                    await self.uow.repository.referrals.update_reward(reward.id, is_issued=True)
                else:
                    # Partial withdrawal: update reward amount to remaining
                    remaining_amount = reward.amount - to_withdraw
                    await self.uow.repository.referrals.update_reward(
                        reward.id, 
                        amount=remaining_amount
                    )
                
                withdrawn_amount += to_withdraw
            
            logger.info(
                f"Withdrew '{withdrawn_amount}' of '{amount}' pending rewards "
                f"for user '{telegram_id}' for type '{reward_type.name}'"
            )
            ...
            logger.info(
                f"Withdrew '{total_amount}' pending rewards "
                f"for user '{telegram_id}' for type '{reward_type.name}'"
            )
            return total_amount

    async def restore_pending_rewards(
        self,
        telegram_id: int,
        reward_type: ReferralRewardType,
        amount: int,
        ...
        """
        Restore previously withdrawn pending rewards (rollback operation).
        Creates a new pending reward entry for the specified amount.
        Used when a payment fails after balance was deducted.
        """
        if amount <= 0:
            return
        
        # Create a new pending reward entry to restore the amount
        reward = await self.uow.repository.referrals.create_reward(
            ReferralReward(
                referral_id=None,  # No referral - this is a restoration
                user_telegram_id=telegram_id,
                type=reward_type,
                amount=amount,
                is_issued=False,  # Not issued - pending
            )
        )
        
        logger.info(
            f"Restored '{amount}' pending rewards for user '{telegram_id}' "
            f"for type '{reward_type.name}' (reward_id={reward.id})"
        )
    ...
    async def create_direct_reward(
        ...
        """Create a direct reward without a referral (admin action).
        
        Note: This method is used for manual admin actions and should NOT
        automatically credit to main balance even in COMBINED mode.
        The reward stays in referral balance until user explicitly transfers it.
        """
        reward = await self.uow.repository.referrals.create_reward(
            ReferralReward(
                referral_id=None,  # No referral - direct admin reward
                user_telegram_id=user_telegram_id,
                type=reward_type,
                amount=amount,
                is_issued=False,  # Always false for admin-added rewards
            )
        )
        
        logger.info(f"Direct reward '{amount}' created for user '{user_telegram_id}'")
    ...
    async def mark_rewards_as_issued(
        self,
        telegram_id: int,
        amount: Decimal,
        reward_type: ReferralRewardType,
    ) -> None:
        """Consume rewards by creating a negative reward (subtraction)."""
        # Create a negative reward to consume/subtract from pending balance
        await self.uow.repository.referrals.create_reward(
            ReferralReward(
                referral_id=None,  # Direct admin action
                user_telegram_id=telegram_id,
                type=reward_type,
                amount=-amount,  # Negative amount for consumption
                is_issued=False,  # Keep as pending so it counts toward balance
            )
        )
        logger.info(
            f"Created negative reward (consumption) of '{amount}' "
            f"for user '{telegram_id}' for type '{reward_type.name}'"
        )
    ...
    async def assign_referral_rewards(self, transaction: TransactionDto) -> None:
        ...
        logger.info(
            f"Assigning rewards for transaction '{transaction.id}', "
            f"purchase_type='{transaction.purchase_type}', "
            f"strategy='{settings.accrual_strategy.name}'"
        )

        # Allow rewards for TOPUP and NEW purchases
        # Skip only for renewals when strategy is ON_FIRST_PAYMENT
        if (
            settings.accrual_strategy == ReferralAccrualStrategy.ON_FIRST_PAYMENT
            and transaction.purchase_type not in [PurchaseType.NEW, PurchaseType.TOPUP]
        ):
            logger.info(
                f"Skip rewards: transaction '{transaction.id}' purchase type "
                f"'{transaction.purchase_type}' is not NEW or TOPUP"
            )
            return
        ...
        for level, referrer in reward_chain.items():
            ...
            # Add currency to reward DTO for notification
            reward.currency = transaction.currency
