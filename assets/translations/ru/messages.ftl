...
Процентная: процент от суммы платежа (для денег) или от дней подписки (для дней).</i>
...
msg-referral-reward =
    ...
    { $reward_type ->
        [EXTRA_DAYS] { $is_days_percent ->
            ...
            *[0] Выберите количество дней за каждые 100₽ пополнения/покупки, или введите значение вручную.
        }
...
msg-referral-cashback =
    ...
    <blockquote>
    Кешбек — процент от награды реферера, который получает реферал (пользователь, совершивший платёж).
    
    Пример: реферал продлевает на 30 дней, награда 10% → реферер получает 3 дня. Кешбек 50% → реферал получает 1 день (50% от 3).
    </blockquote>
